 ...
 * 
 * Cross-references TikTok post analytics (via PosteAhora) with RevenueCat
 * conversions to identify which hooks drive views AND revenue.
 *
 * Data sources:
 * 1. PosteAhora API → per-post TikTok analytics (views, likes, comments, shares)
 * 2. PosteAhora API → platform-level engagement totals for delta tracking
...
const POSTEAHORA_URL = process.env.POSTEAHORA_API_URL || 'https://api.posteahora.com/functions/v1/api';
const API_KEY = process.env.POSTEAHORA_API_KEY || config.posteahora?.apiKey;

async function posteahoraAPI(endpoint) {
  const res = await fetch(`${POSTEAHORA_URL}${endpoint}`, {
...
(async () => {
  ...
  // ==========================================
  // 1. PosteAhora: per-post TikTok analytics
  // ==========================================
  // PosteAhora returns per-post metrics directly (it stores each post's remote
  // TikTok id internally), so there is no video-id reconciliation to do here.
  const analytics = await posteahoraAPI(`/analytics?period=${periodForDays(days)}&platform=tiktok`);
  const startMs = startDate.getTime();
  const rows = (analytics.posts || [])
    .filter(p => !p.publishedAt || new Date(p.publishedAt).getTime() >= startMs)
    .sort((a, b) => new Date(b.publishedAt) - new Date(a.publishedAt));

  console.log(`  📱 Found ${rows.length} TikTok posts with analytics\n`);

  const postResults = rows.map(p => ({
    id: p.postId,
    date: (p.publishedAt || '').slice(0, 10),
    hook: (p.caption || '').substring(0, 70),
    app: config.app?.name || '',
    views: p.views || 0,
    likes: p.likes || 0,
  ...
  // ==========================================
  // 2. PosteAhora: platform-level engagement totals (delta tracking)
  // ==========================================
  // The same analytics call already aggregates per-platform totals.
  const platformStats = {};
  const byPlatform = (analytics.summary && analytics.summary.byPlatform) || {};
  ...
  if (prevPlatformStats) {
    ...
    for (const [platform, stats] of Object.entries(platformStats)) {
      ...
      if (prev) {
        ...
        const likeDelta = (stats.likes || 0) - (prev.likes || 0);
        report += `**${platform}:** +${viewDelta.toLocaleString()} views, +${likeDelta.toLocaleString()} likes\n`;
      } else {
        report += `**${platform}:** ${(stats.views || 0).toLocaleString()} views, ${(stats.likes || 0).toLocaleString()} likes\n`;
      }
    }
    report += '\n';
  }
