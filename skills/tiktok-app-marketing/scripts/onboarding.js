...
if (init) {
  ...
  const configTemplate = {
    ...
    posteahora: {
      // API key from PosteAhora → Settings → API & integrations (pah_live_…)
      apiKey: '',
      // Account ids from `posteahora accounts` (or GET /accounts)
      accountIds: {
        tiktok: '',
      },
    },
    ...
    posting: {
      // TikTok posts are created as drafts; add a trending sound, then publish.
      status: 'draft',
...
if (validate && configPath) {
  ...
  // PosteAhora (required)
  if (!config.posteahora?.apiKey) required.push('posteahora.apiKey — PosteAhora API key (pah_live_…)');
  if (!config.posteahora?.accountIds?.tiktok) {
    required.push('posteahora.accountIds.tiktok — TikTok account id (run `posteahora accounts`)');
  }
  ...
  console.log(`   TikTok: ${config.posteahora?.accountIds?.tiktok ? 'Connected' : 'Not connected'}`);

  const crossPost = Object.keys(config.posteahora?.accountIds || {}).filter(
    (k) => k !== 'tiktok' && config.posteahora.accountIds[k],
  );
  ...
  console.log(`   Post status: ${config.posting?.status || 'draft'}`);
