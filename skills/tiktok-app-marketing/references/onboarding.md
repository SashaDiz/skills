...
The conversational setup flow for a new user. Run this once, when the skill is first loaded — not on every post. Talk like a human marketing partner: one or two questions at a time, react to the answers, don't dump a checklist.

Finish with `scripts/onboarding.js --validate` to confirm the config is complete.
